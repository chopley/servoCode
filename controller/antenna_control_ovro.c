...
void velocity_pid(long vel_tacho, double vel_command, double position_error,double Kfa, double Pd, double Id, double Dd, long Mx_I, long Mn_I, int MAX_OUT,int MIN_OUT,double *current_i_ptr,long *velpid_out, long *pid_return_ptr);
...
void *ovro_thread(void *arg){
	...
	int sd,rc,n, cliLen,read_ret,childpid,newsockfd;
	int i,ij;
	int childfd;
   ...
   */
   //make sure that everything is clear in memory.
	...
	LOCAL_SERVER_PORT_OVRO);
	if (listen(sd, 1) < 0){ /* allow 5 requests to queue up */ 
	...
	//do necessary housekeeping to set up the communications
	    ...
	    printf("server established connection with %s (%s) %u\n",hostp->h_name, hostaddrp,LOCAL_SERVER_PORT_OVRO);
	//we have a socket- now spawn a process in case it gets disconnected from outside   
	    n=1;
	    childpid = fork();
	    if(childpid< 0)
		error("Failed fork\n");
	    ...
	    if(childpid == 0){
		 ...
		 while(n>0){
		    ...
		    printf("connected to control programm\n");
			...
			printf("GAE command received\n");
		      //put together the string to reply with
			sprintf(return_string,"%s,%f,%f\r",GAE,readout.az_ready_to_read[0],readout.alt_ready_to_read[0]);
		    }
		    else if(!strcmp(commands[0],GIM)){
			printf("GIM command received\n");
			//put together the string to reply with
			sprintf(return_string,"%s,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\r",GIM,readout.az_ready_to_read[0],readout.az_ready_to_read[1],readout.az_ready_to_read[2],readout.az_ready_to_read[3],readout.az_ready_to_read[4],readout.alt_ready_to_read[0],readout.alt_ready_to_read[1],readout.alt_ready_to_read[2],readout.alt_ready_to_read[3],readout.alt_ready_to_read[4],readout.az_err_ready_to_read[0],readout.az_err_ready_to_read[1],readout.az_err_ready_to_read[2],readout.az_err_ready_to_read[3],readout.az_err_ready_to_read[4],readout.alt_err_ready_to_read[0],readout.alt_err_ready_to_read[1],readout.alt_err_ready_to_read[2],readout.alt_err_ready_to_read[3],readout.alt_err_ready_to_read[4]);
		    }
	   ...
	   //if(readout.ready==1){
		//readout.ready=0;
		//for(i=0;i<5;i++){
		  //  printf("Readout is %f %f\n",readout.az_ready_to_read[i],readout.alt_ready_to_read[i]);
	//	}
	  //  }
	      
		    printf("server received %d bytes: %s \n", n, buf);
		    printf("Returning a string made up like this %s \n",return_string);
		    n = send(childfd, return_string, 1024,0);
		    if (n < 0){ 
		...
		close(childfd);
		waitpid(childpid, NULL, 0); //check the return status and cleanup zombies
...
void *command_thread(void *arg){
  ...
  bzero((char *) &servAddr, sizeof(servAddr));
	...
	printf("Received Structure %d\n",sizeof(tcp_message));
	 n = send(childfd, &tcp_message.message_size, sizeof(tcp_message.message_size),0);
    	if (n < 0) 
	  error("ERROR writing to socket");
	...
	case HORIZONTAL_LIST:
		...
		if(tcp_message.lcommand_vals[0]>0){
		  ...
		  control.eq2_time_begin = tcp_message.lcommand_vals[1];
		  control.coordinate_command_type=coordinate_type;
		  for(i=0; i<control.eq2_time_end-control.eq2_time_begin; i++){
		...
		pthread_mutex_unlock (&mutexsum);
		for(i=0; i<5; i++){  
		  printf("i = %d Az %f %f Alt %f %f %ld %ld\n",i,command.comaz[i],tcp_message.az_commands[i],command.comalt[i],tcp_message.alt_commands[i],control.eq2_time_end,control.eq2_time_begin);
		}
	break;
...
//	ioctl(fd,DEV_IOCTL_READ_CONTROL_STRUCTURE, &controlc);
	...
	while(1){
	  printf("In the coordinate timing update thread\n");
	ioctl(fd,DEV_IOCTL_READ_CONTROL_STRUCTURE, &controlc);
	usleep(1000000000);
	status_query(&control.status_vec);
...
// 	}
	...
	printf("Left the coordinate timing update thread\n");
	}
...
//LOOP BEGINS HERE!!-------------------------------	    
	  ...
	  while(1){ //probably don't need this while statement anymore? Can just be while(1) ?
	    ...
	    //-------------TRY THIS HERE RATHER THAN IN THE COORDINATE INTERRUPT-This relies on one type of command being given to the controller-
			...
			if((control.coordinate_command_type==EQUATORIAL2) || control.coordinate_command_type==HORIZONTAL_LIST ){
				...
				if(control.coordinate_command_type==EQUATORIAL2){
				 ...
				 if(control.coordinate_command_type==HORIZONTAL_LIST){
				   i = current_time-control.eq2_time_begin;
				   if(i>(control.eq2_time_end-control.eq2_time_begin-2)){
				    i=control.eq2_time_end-control.eq2_time_begin- 2 ;
				    //printf("Overflow %d \n",i);
				   }
				...
				if(vel_az > MAX_AZ_POS_SPACE_DEG){
				 azimuth_val=azimuth_val-vel_az +MAX_AZ_POS_SPACE_DEG; 
				}
...
// 				
				...
				//azimuth_angle(loop.azimuth_command_double,loop.az_encoder_long,AZIMUTH_ZERO,AZIMUTH_SAFETY_LO,AZIMUTH_SAFETY_HI,&loop.azimuth_command_double);
				  ...
				  if(counterb==0){
				    printf("Eq2 Error-Need new quadratic coefficients to maintain pointing accuracy-Counterb. Tend = %ld T= %ld\n",control.eq2_time_end,current_time);
				  counterb=1;
				  }
				  loop.vel_of_az = 0;
				  loop.vel_of_alt = 0;
	    ...
	    if(control.updatepos ==5){//update the control angles- these are done in the coordinate thread once a second after which the flag control.updatepos is set. This if statement checks whether the flag is set (i.e whether commands need to be updated?) and then proceeds to copy relevent values into relevent places.
		    control.updatepos=0;
	  ...
	  //  update_pid_double(AZ_PID, loop.azimuth_command_double,prev_azencoderd, loop.azimuth_encoder_double,loop.pcoeffs[0],loop.icoeffs[0],loop.dcoeffs[0],loop.kfcoeffs[0],loop.vel_of_az,time_diff,loop.motor_plus[0],loop.motor_minus[0],MAX_AZ_SPEED,MIN_AZ_SPEED,3500/loop.icoeffs[0],-3500/loop.icoeffs[0], &azerr1, &loop.az_ic1,&loop.az_pid1);
	   ...
	   // update_pid_double(ALT_PID, loop.altitude_command_double,prev_altencoderd, loop.altitude_encoder_double,loop.pcoeffs[2],loop.icoeffs[2],loop.dcoeffs[2],loop.kfcoeffs[2],loop.vel_of_alt,time_diff,loop.motor_plus[2],loop.motor_minus[2],MAX_ALT_SPEED,MIN_ALT_SPEED,3000/loop.icoeffs[2],-3000/loop.icoeffs[2], &alterr1, &loop.alt_ic1,&loop.alt_pid1);
...
//	VELOCITY PID LOOP----------------------	   
	 ...
	 // if((loop.kfcoeffs[0]*loop.vel_of_az<10000.) && (loop.kfcoeffs[0]*loop.vel_of_az>-10000.)){
		...
		pid_return_new[0] = loop.az_pid1;
		pid_return_new[1] = 0.5*loop.az_pid1;
		//printf("aa pid_return %d %d\n",loop.az_pid1,loop.alt_pid1);
		aztacho[0] = (long)aztacho1; //use the low velocity value from the ADC i.e channel 1
 		kfa_azimuth = loop.kfcoeffs[0];
		velocity_pid(aztacho[0],loop.vel_of_az,azerr1,kfa_azimuth,loop.pcoeffs_vel[0],loop.icoeffs_vel[0], 0.,1300,-1300,2000,-2000,&loop.az_ic1_vel,&velpid_out_az,&loop.az_pid1);
		//if(countera==9){
 		 // printf("Aztacho1 Zone %d %f %f %f\n",aztacho[0],loop.vel_of_az,kfa_azimuth,loop.vel_of_az*kfa_azimuth);
 		 //}
		break;
	      case 0: //i.e the high velocity zone
		update_pid_double(AZ_PID, loop.azimuth_command_double,prev_azencoderd, loop.azimuth_encoder_double,loop.pcoeffs[1],loop.icoeffs[1],loop.dcoeffs[1],loop.kfcoeffs[1],loop.vel_of_az,time_diff,loop.motor_plus[1],loop.motor_minus[1],MAX_AZ_SPEED,MIN_AZ_SPEED,3500/loop.icoeffs[1],-3500/loop.icoeffs[1], &azerr1, &loop.az_ic1,&loop.az_pid1);
		//printf("bb pid_return %d %d\n",loop.az_pid1,loop.alt_pid1);
		pid_return_new[0] = loop.az_pid1;
		pid_return_new[1] = 0.5*loop.az_pid1;
		
		aztacho[0] = (long)aztacho2;
 		kfa_azimuth = loop.kfcoeffs[1];
		velocity_pid(aztacho[0],loop.vel_of_az,azerr1,kfa_azimuth,loop.pcoeffs_vel[1],loop.icoeffs_vel[1], 0.,2000,-2000,3000,-3000,&loop.az_ic1_vel,&velpid_out_az,&loop.az_pid1);
		//printf("Aztacho2 Zone %d %f\n",aztacho[0],loop.vel_of_az);
		//if(countera==9){
 		 // printf("Aztacho2 Zone %d %f %f\n",aztacho[0],loop.vel_of_az,kfa_azimuth);
 		// }
		break;
	      default:
	   ...
	   switch((loop.vel_of_alt<50. && loop.vel_of_alt>-50.)){
		...
		velocity_pid(alttacho[0],loop.vel_of_alt,alterr1,loop.kfcoeffs[2],loop.pcoeffs_vel[2], loop.icoeffs_vel[2], 0.,1000,-1000,2000,-2000,&loop.alt_ic1_vel,&velpid_out_alt,&loop.alt_pid1);
		//if(countera==9){
 		 // printf("Alttacho1 Zone %d %f %f\n",aztacho[0],loop.vel_of_az,kfa_azimuth);
 		// }
		break;
	      case 0: //i.e the high velocity zone
		update_pid_double(ALT_PID, loop.altitude_command_double,prev_altencoderd, loop.altitude_encoder_double,loop.pcoeffs[3],loop.icoeffs[3],loop.dcoeffs[3],loop.kfcoeffs[3],loop.vel_of_alt,time_diff,loop.motor_plus[3],loop.motor_minus[3],MAX_ALT_SPEED,MIN_ALT_SPEED,3000/loop.icoeffs[3],-3000/loop.icoeffs[3], &alterr1, &loop.alt_ic1,&loop.alt_pid1);
		pid_return_new[2] = loop.alt_pid1;
		pid_return_new[3] = 0.5*loop.alt_pid1;
		
		alttacho[0] = (long)alttacho2;
		velocity_pid(alttacho[0],loop.vel_of_alt,alterr1,loop.kfcoeffs[3],loop.pcoeffs_vel[3], loop.icoeffs_vel[3], 0.,1000,-1000,2000,-2000,&loop.alt_ic1_vel,&velpid_out_alt,&loop.alt_pid1);
		break;
	      default:
	    ...
	    readout.alt_pos_err_table[2] = azerr1; 
		...
		else 
		{
		   ...
		   // for(j=0;j<readout.sample_number;j++)
		   // {
		     //  printf("Sample %ld %d %f \n", readout.time[0],j,readout.az_ready_to_read[j]);
		   // }
		 }
	     readout.current_value=0; 
	     readout.sample_number =0 ;
	    ...
	    if(readout.azimuth_time_table[1]<=readout.current_value & readout.azimuth_time_table[2]>readout.current_value){
	      ...
	      //printf("One second %ld %f %ld %f %ld %f interpolated %f\n",readout.azimuth_time_table[0],readout.azimuth_position_table[0],readout.azimuth_time_table[1],readout.azimuth_position_table[1],readout.azimuth_time_table[2],readout.azimuth_position_table[2],readout.az_position[0]); 
	     // printf("%f %f %f %f %f %f %f %f\n ",readout.calc_time[3],readout.calc_time[4],readout.calc_time[5],readout.calc_time[6],readout.calc_time[7],readout.calc_time[8],readout.calc_time[9],readout.az_position[0]);
	    }
...
//-----------------------------------------------------
	      if(countera==9){
	      // printf("%7ld \n",loop.az_err1);
	       printf("AZ PID pos %7d vel %7d Tot %7d AZ %7lf %7lf %7lf %7d %7lf %7lf %7lf ",pid_return_new[0],velpid_out_az,loop.az_pid1,loop.vel_of_az,loop.kfcoeffs[0]*loop.vel_of_az,loop.kfcoeffs[1]*loop.vel_of_az,aztacho1,azencoder_vel[7],loop.vel_of_alt,azerr1);
	       printf("ALT PID pos %7d vel %7d Tot %7d AZ %7lf %7lf %7lf %7d %7lf %7lf %7lf \n",pid_return_new[2],velpid_out_alt,loop.alt_pid1,loop.vel_of_alt,loop.kfcoeffs[2]*loop.vel_of_alt,loop.kfcoeffs[3]*loop.vel_of_alt,alttacho1,altencoder_vel[7],loop.vel_of_alt,alterr1);
...
//-------------------------------------------------	    
		...
		pid_return_new[0] -= loop.vfcoeffs[0]*aztacho2;
		pid_return_new[1] -= loop.vfcoeffs[0]*aztacho2;
	      }
	    }
	    
	    if(loop.vel_of_alt<50. && loop.vel_of_alt>-50.){
	       if(alterr1 <1. && alterr1 >-1.){
		temp_double =  loop.vfcoeffs[2]*alttacho2;
		/////////////////////////////////////
		...
		//pid_return_new[2] -= loop.vfcoeffs[2]*alttacho2;
		//pid_return_new[3] -= loop.vfcoeffs[2]*alttacho2;
    ...
    //	backlash(user.alt_command_long,user.alt_encoder_long,2000,0,0,0,&user.alt_pid1,&user.alt_pid2);
	  ...
	  soft_lim(&loop,loop.limits[1],loop.limits[0],AZIMUTH_SAFETY_HI,AZIMUTH_SAFETY_LO,AZ_SLOW_SPEED_POSITIVE,AZ_SLOW_SPEED_NEGATIVE,user.az_encoder_long,&(loop.az_command_long),&(loop.az_pid),&(pid_return_new[0]),&(pid_return_new[1]));//the software limit checking to prevent antenna driving into its own hard limits.
	    ...
	    backlash(loop.az_command_long,user.az_encoder_long,loop.vel_of_az,500,100,0,0,&pid_return_new[0],&pid_return_new[1]);
	      backlash(loop.alt_command_long,user.alt_encoder_long,loop.vel_of_alt,500,100,0,0,&pid_return_new[2],&pid_return_new[3]);
