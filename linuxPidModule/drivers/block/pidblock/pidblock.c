...
static unsigned int sbull_major;
...
static const struct file_operations dev_ops= { /* Operations for controling the device */
        .owner             = THIS_MODULE,
        .read              = user_pid_read,
        .write             = user_pid_write,
	.open		   =user_pid_open,
...
//------------------------------------------------------
	...
	//	__raw_writel();
		...
		__raw_writel(AT91_TC_TIMER_CLOCK1,timer0_base+AT91_TC_CMR);
		__raw_writel(AT91_TC_COVFS,timer0_base+AT91_TC_IER);
...
// 	REGISTER A ADC DEVICE
	...
	sbull_major=register_blkdev(sbull_major, "PIDBLOCK");
	if(returnval <= 0)
	{PDEBUG("Problem registering PIDBLOCK\n");
	}
	else{
	sbull_major=returnval;
	PDEBUG("PIDBLOCK MAJOR NODE = %u\n",sbull_major);
	}
...
static int mod_exit(void){
	...
	unregister_blkdev(sbull_major, "PIDBLOCK");
