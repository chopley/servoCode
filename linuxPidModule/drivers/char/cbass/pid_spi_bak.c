...
struct ad5362_driver{
	...
	unsigned int ncpha,cpol,mstr,spien,ps,pcsdec,dlybcs,pcs,lastxfer,csaat,bits,scbr,dlybs,dlybct;
...
static int mod_init(void){
	...
	PDEBUG("Return from ad5362_pci %u\n",ret);
...
int dac5362_write(int dac_number, unsigned short dacvalue){
	...
	receive[4]=0x33;
	receive[5]=0x23;
	//at91_set_GPIO_periph(ad5362.ldac,0);
	//at91_set_GPIO_periph(ad5362.reset,0);
	//at91_set_GPIO_periph(ad5362.clear,0);
	...
	at91_set_gpio_value(ad5362.ldac,0);
	at91_set_gpio_value(ad5362.reset,1);
	at91_set_gpio_value(ad5362.clear,1);
	
	address = virt_to_bus(ad5362.TxBuffer);
//	AT91SAM9260_BASE_SPI1
	...
	//__raw_writel(ad5362.pcs,spi1_base+AT91_SPI_TDR);
	...
	//__raw_writel((1<<24)|(0<<19)|(0<<18)|(0<<17)|(0<<16)|(0x0000aaaa),spi1_base+AT91_SPI_TDR);
	

//  	transmit[0]=0xffffffffffffffff;
// // 	transmit[1]=(1<<24)|(0<<19)|(0<<18)|(0<<17)|(0<<16)|(0x0000aaaaa);
// // 	//transmit[2]=(1<<24)|(0<<19)|(0<<18)|(0<<17)|(0<<16)|(0x00001e1f);
  	transmit_char[0]=0xaa;
  	transmit_char[1]=0xaa;
  	transmit_char[2]=0xaa;
// // 	__raw_writel(ad5362.spien,spi1_base+AT91_SPI_CR);
// // // 	//enable transmit and receive of the PDC
 	__raw_writel((1<<8|1<<1),spi1_base+SPI_PTCR);
// // 	//load a pointer to the data
   	__raw_writel(address,spi1_base+SPI_TPR);
//  	//__raw_writel((unsigned int)receive,spi1_base+SPI_RPR);
// // 	//length of data is 3 bytes
// // 	
// // 	//no next trasnmit pointer
// 	//__raw_writel((unsigned int)transmit,spi1_base+SPI_TNPR);
//  	__raw_writel(0,spi1_base+SPI_TNCR);
// // 	//__raw_writel((unsigned int)receive,spi1_base+SPI_RNPR);
...
// 	__raw_writel(3,spi1_base+SPI_RCR);
// 	//no next trasnmit pointer
// 	__raw_writel(0,spi1_base+SPI_TNPR);
// 	__raw_writel(0,spi1_base+SPI_TNCR);
	
	//__raw_writel(ad5362.spien,spi1_base+AT91_SPI_CR);
		
	//PDEBUG("rxbuffer %02x %02x %02x %02x %02x %02x \n",receive[0],receive[1],receive[2],receive[3],receive[4],receive[5]);
