...
unsigned int read_spi_max7301(unsigned int chip1,unsigned int chip2,unsigned int number,volatile unsigned int *rxbuff){
	...
	unsigned int return_val;
	int i;
	volatile unsigned int buffer1[100];
	volatile unsigned int buffer2[100];
	unsigned long status=0x0000;
	unsigned long count = 0x0000;	
	unsigned long irq_state;
...
//	datacom[0]=(control_packet>>24)&0x000000ff;
	...
	dma_sync_single(dev_ptr, dma_address2, 100,DMA_FROM_DEVICE);
	dma_sync_single(dev_ptr, dma_address1, 100,DMA_TO_DEVICE);
	...
	//----------------------------------------------------
		...
		dma_unmap_single(dev_ptr, dma_address1, 100,DMA_TO_DEVICE);
		dma_unmap_single(dev_ptr, dma_address2, 100,DMA_FROM_DEVICE);
		//this is VERY NB!! Must synch data or it doesn't go through!
		dma_sync_single(dev_ptr, dma_address2, 100,DMA_FROM_DEVICE);
		dma_sync_single(dev_ptr, dma_address1, 100,DMA_TO_DEVICE);
...
//	return_val=(((buffer4[3]<<16)&(0x00ff0000))|((buffer4[4]<<8)&(0x0000ff00))|(buffer4[5]&(0x000000ff)));
	...
	udelay(100);
	*rxbuff = buffer2[0]&0x00ff;
	*(rxbuff+1) = buffer2[1]&0x00ff;
	//switch these two around as they are returned in the order chip 2 followed by chip1
	*(rxbuff+2) = buffer2[3]&0x00ff;
	*(rxbuff+3) = buffer2[2]&0x00ff;
	*(rxbuff+4) = buffer2[4]&0x00ff;
	*(rxbuff+5) = buffer2[5]&0x00ff;
	
	//printk("READ MAX7301in %08x %08x %08x %08x %08x %08x\n",(buffer2[0]&0x00ff),(buffer2[1]&0x00ff),(buffer2[2]&0x00ff),(buffer2[3]&0x00ff),(buffer2[4]&0x00ff),(buffer2[5]&0x00ff));
