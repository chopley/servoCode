...
//	control_packet = 0x00c00000|(dac_out&(0x0000ffff));
	...
	if((void *)dma_address1==NULL||(void *)dma_address2==NULL){
		printk("AD7367 DMA-ADDRESS ALLOCATION FAILED\n");
	}
...
//	printk("DMA address %08x %08x\n",dma_address1,dma_address2);
	...
	//	__raw_writel(txbuffer[0],spi1_base+AT91_SPI_TDR);
		...
		while( (!(status&AT91_SPI_RXBUFF)) && (count<500)   ) {
			...
			count++;
			if(count>498){
				printk("ERROR: AD7367 SPI TRANSMIT TIME OUT1- NO TRANSFER\n");
	...
	test[0] = *rxbuffer&(0x00003fff);
	test[1]= *(rxbuffer+1)&(0x00003fff);
	test[2]=test[0]<<2;
	test[3]=test[1]<<2;
	*(output) = (int)test[2];
	*(output+1) = (int)test[3];
//	printk("sign1 %d sign2 %d test[0] %d test[1] %d\n",sign1,sign2,test[2],test[3]);
	return 0;
}
...
//	__raw_writel(1,spi1_base+SPI_TCR);
	...
	while( (!(status&AT91_SPI_RDRF)) && (count<500)   ) {
		...
		count++;
		if(count>498){
			printk("ERROR: AD7367 SPI TRANSMIT TIME OUT- NO TRANSFER\n");
			PDEBUG("count %lu SPI1_SR %08lx\n",count,status);
			}
