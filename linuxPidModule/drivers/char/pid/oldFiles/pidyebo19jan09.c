...
static irqreturn_t pid_irq(int irq, void *_cf, struct pt_regs *r)
{
	...
	controli = control;
	spin_unlock(&mylock);
	if(timer_counter >=controli.interrupt_rate){
		
		daccounter++;
		timer_counter=0;
		controli.counter++;
		if(controli.counter == 10){
			ret = emergency_stop(&controli);
	...
	//PDEBUG("%lu\n",controli..alt_adc);
		if(readflag==1 ){
		//	printk("Readflag %d %d %d %d\n",readflag,controli..coordinate_command_type,controli..az_command_long,controli..alt_command_long);
			if(controli.update==1){
				spin_lock(&mylock);
				ret = update();
				spin_unlock(&mylock);
			}
			...
			readflag =0;
			flag = 1;
			controli.time=__raw_readl(timer0_base+AT91_TC_CV);  
			if(controli.azimuth_zone<=2){
			
			

			//get the encoder and tachometer readings from the antenna
			get_antenna_readings(&controli.az_encoder_long,&controli.az_encoder,&controli.alt_encoder_long,&controli.alt_encoder,&controli.azimuth_zone, &controli.tacho1,&controli.tacho2,&controli.tacho3,&controli.tacho4);
		//	printk("AZ Encoder %d ALT Encoder %d\n",controli.az_encoder_long,controli.alt_encoder_long);

		//	printk("AZ command %d ALT command %d\n",controli.az_command_long,controli.alt_command_long);
		//	soft_lim = soft_limits(controli.azimuth_zone,controli.az_encoder_long,controli.alt_encoder_long,controli.az_command_long,controli.alt_command_long,&com_az_temp,&com_alt_temp);
			//soft_lim =0;
			//tacho_lim =tacho_check(controli.tacho1,controli.tacho2,controli.tacho3,controli.tacho4,MAX_TACHO_ALLOWABLE,MIN_TACHO_ALLOWABLE);
			//printk("AZ command %d ALT command %d\n",controli.az_command_long,controli.alt_command_long);
			//controli.az_command_long = com_az_temp;
			//controli.alt_command_long = com_alt_temp;
			
			soft_lim=0;
			tacho_lim=0;
			if(soft_lim==0){
				//printk("%d %d %d %d\n",controli.az_pid1,controli.az_pid2,controli.alt_pid1,controli.alt_pid2);
				ret=dac5362_crc(WRITE_AD5362,controli.az1_dac_control);	
				ret=dac5362_crc(WRITE_AD5362,controli.az2_dac_control);
				ret=dac5362_crc(WRITE_AD5362,controli.alt1_dac_control);
				ret=dac5362_crc(WRITE_AD5362,controli.alt2_dac_control);
	
				//make sure the DAC registers do not become corrupted- I'm sure there is a better way!
				for(i=0;i++;i<=7){
					ret=dac5362_crc(WRITE_AD5362,controli.cbuffer_crc[i]);
					ret=dac5362_crc(WRITE_AD5362,controli.mbuffer_crc[i]);
				}
				controli.cbuffer_read_ret[0]=0;
				controli.cbuffer_read_ret[0] = dac5362_crc(READ_AD5362,controli.cbuffer_crc_read[0]);
			}
			else if(soft_lim!=0){
			...
			controli.counter = 0;
			do_gettimeofday(&(controli.time_struct));
			spin_lock(&mylock);
			control = controli;
			spin_unlock(&mylock);
